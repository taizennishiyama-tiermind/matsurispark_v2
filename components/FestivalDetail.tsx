        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 text-center">
            {sponsors.map(sponsor => (
                <div key={sponsor.id} className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg flex flex-col items-center justify-center border border-slate-200 dark:border-slate-700 transition-transform transform hover:scale-105">
                    <img src={sponsor.logoUrl} alt={`${sponsor.companyName} logo`} className="h-20 w-20 rounded-full object-cover mb-3 shadow-md" />
                    <p className="font-bold text-sm text-slate-800 dark:text-white">{sponsor.companyName}</p>
...
export const FestivalDetail: React.FC<FestivalDetailProps> = ({ festival, onBack }) => {
    ...
    const handleFormSubmit = () => {
        setSelectedTier(null); // Close form on successful submission
    };
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl overflow-hidden">
                <img src={festival.imageUrl} alt={festival.name} className="w-full h-64 md:h-96 object-cover" />
                <div className="p-6 md:p-10">
                    {isGoalBased && (
                        <div className="mb-8 p-6 bg-slate-100 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700">
                            ...
                            <div className="flex justify-between items-end mt-2 text-slate-700 dark:text-slate-300">
                                <div>
                                    <p className="font-bold text-2xl text-cyan-500">¥{(festival.currentFunding || 0).toLocaleString()}</p>
                                    <p className="text-sm">現在の支援総額</p>
                                </div>
                                <div className="text-right">
                                    <p className="font-bold text-lg">目標金額: ¥{(festival.fundingGoal || 0).toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <p className="text-slate-700 dark:text-slate-400 text-lg leading-relaxed whitespace-pre-wrap">{festival.longDescription}</p>

                    {festival.sponsors && festival.sponsors.length > 0 && (
                        <>
                            <div className="my-10 border-t border-slate-200 dark:border-slate-700"></div>
                            <SponsorList sponsors={festival.sponsors} />
                        </>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {festival.sponsorshipTiers.map(tier => (
                    ...
                    {selectedTier && (
                        <div id="sponsorship-form" className="mt-12">
                            <SponsorshipForm tier={selectedTier} festivalName={festival.name} onSubmitSuccess={handleFormSubmit} />
                        </div>
                    )}
                </div>
            </div>
